...
async def get_profile(user_id: str = Path(...)):
    ...
    try:
        ...
        return {
            ...
            "custom_instructions": created.get("custom_instructions", ""),
...
async def update_profile(
    ...
    try:
        ...
        if req.custom_instructions is not None:
            update_data["custom_instructions"] = req.custom_instructions
...
async def list_chapters(subject: str, grade: int):
    ...
    if not res.data:
        return {
            "chapters": [],
            "message": f"No verified content for {subject} Grade {grade} yet",
        }
