"""
Batch ingest ONLY NEW subject PDFs into Supabase pgvector.
Does NOT re-ingest Science, Math, Eco (already done).

Usage:
  cd backend
  python -m scripts.batch_ingest_new
"""
import glob
import subprocess
import sys
...
# Map ONLY new subject folders to subjects
NEW_SUBJECTS = {
    "9-Eng": "English",
...
def main() -> None:
    ...
    for folder_prefix, subject in NEW_SUBJECTS.items():
        ...
        for i, pdf_path in enumerate(pdfs, 1):
            ...
            print(f"[{i}/{len(pdfs)}] {pdf_name}...", end=" ", flush=True)
            total_pdfs += 1
    ...
    print(f"\n{'='*60}")
    print(f"📊 NEW SUBJECTS INGESTION SUMMARY")
    print(f"{'='*60}")
